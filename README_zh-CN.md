...
> 反映 2026 年 9 月 DeepSeek 官方公告中的计费策略（`policyVersion "2026-09"`）：
> 上述时段窗口**仅在工作日（周一至周五）生效**；**周末两天（周六、周日）
> 全 off-peak**，与具体时刻无关。
...
（`PeakPeriodTests`，以北京时间为锚的 21 个边界用例，其中 6 个覆盖 issue #19
`policyVersion "2026-09"` 引入的周末短路规则）、SwiftUI 视图，
