...
**一句话描述**：一款 macOS 菜单栏小工具，定时抓取 LLM API 平台的用量数据，同时支持周期配额型和余额型两种监控模式，并以紧凑的进度条 + 文字形式在菜单栏正方形图标内直观展示，帮助开发者实时掌握 API 消费情况，避免超出配额或预算。
...
- 常驻 macOS 顶部菜单栏
- **无配置状态**：未添加任何服务实例时，菜单栏显示提示符号 `?`，左键点击打开用量面板 Popover（空状态），通过 Popover 内的入口进入设置窗口
- **有配置状态**：按以下规则显示槽位：
  - 每个启用的服务实例占用一个槽位，槽位尺寸固定为 **高 22pt × 宽 44pt**（2 个正方形宽度）
  - 启用 1~2 个实例时，菜单栏直接显示对应数量的槽位（1 个或 2 个）
  - 启用 **≥3 个实例**时，菜单栏**仅显示前 2 个槽位**，其余实例不显示，必须点击打开 Popover 查看
...
- **启动刷新中状态**：应用启动后尚未完成首次刷新时，所有已启用实例的槽位以灰色像素字模显示 `•••`（3 个圆点），进度条置灰，等待首次刷新完成后切换为实际数据
- **全部禁用状态**：所有已配置实例均被禁用时，菜单栏以灰色像素字模显示 `NO API`，左键点击仍可打开 Popover 进入设置
- **余额不可用状态**：余额型实例 API 返回 `is_available = false` 时，该实例槽位置灰，以灰色像素字模显示 `N/A`；此状态在下次刷新返回 `is_available = true` 后自动恢复
- **图标为代码动态绘制**，不依赖静态 SVG/PNG 资源文件，随数据刷新实时重绘
- 每个槽位采用**单行连续布局**，内容水平紧凑排列，根据实例类型区分：
  - **周期配额型实例**：**显示名简称**（左，2 个英文字母）+ **微型进度条**（中，约 3pt 高、18pt 宽）+ **百分比数字**（右，如 `82%`）
  - **余额型实例**：**显示名简称**（左，2 个英文字母）+ **余额数值**（右，如 `¥45`），**不显示进度条**
- **槽位内所有文字（简称、数字、`%`、`¥`、`.` 等符号）均使用手写像素字模，通过代码逐像素绘制**，不依赖任何字体文件或系统字体渲染。字符集覆盖 A-Z、0-9 及必要半角符号，每个字符以 5×7（或更紧凑的 3×5 数字）像素矩阵硬编码，确保在 22pt 高度内绝对清晰、边缘锐利，不受抗锯齿或屏幕缩放影响
- 图标色彩方案（用户可配置）：
  - **单色模式**：图标跟随 macOS 菜单栏明暗主题（黑/白）。阈值状态通过**进度条填充比例**编码：0–50% 空心线框、50–80% 半实心填充、80–100% 全实心填充，达到严重线时整槽以 1Hz 频率闪烁。余额型实例在单色模式下不闪烁，仅通过数值本身传递信息
  - **彩色模式**：每个槽位独立着色，颜色规则与各服务实例绑定的阈值一致（安全/警告/严重对应不同颜色）
- 左键点击图标始终弹出用量详情面板 Popover；右键菜单栏图标：快速操作 —— 立即刷新 / 打开设置 / 退出

### 3.2 用量面板

- 点击菜单栏图标弹出 Popover 面板，按**服务实例**分组展示用量卡片。同一供应商的不同统计维度（如 MiniMax 文本 5h 与 MiniMax 非文本每日）各自独立一张卡片
- 每张卡片展示：
...
- **错误摘要栏**：当存在刷新失败的实例时，Popover 顶部显示非阻塞提示，区分错误类型：
...
|--------|----------|------|----------|---------------|
| MiniMax | 文本模型请求数 | 周期配额型 | `GET /v1/token_plan/remains` | 5 小时滚动窗口 |
...
- **刷新失败时 Popover 处理**：在面板顶部显示错误摘要栏，区分错误类型（网络异常 / API Key 失效 / 服务端错误等），并提供"立即重试"按钮
...
**首次使用提示**
- 应用首次启动且无配置时，菜单栏显示 `?` 符号；左键点击始终打开用量面板 Popover，Popover 内展示空状态（Empty State），包含「添加第一个服务」按钮，点击后进入设置窗口

**服务实例管理**
- 添加实例：选择供应商 + 统计维度（如「MiniMax → 文本模型 5h」「DeepSeek → 余额」）
- 每个实例可配置：
  - **显示名**：自定义名称（如「MiniMax-文字」「DS-主号」），**默认空**
  - **显示名简称**：2 个英文字母，用于菜单栏槽位，**默认空**
  - **排序序号**（`sort_order`）：整数，升序排列，决定菜单栏槽位和 Popover 卡片的显示顺序；默认按创建顺序分配（0, 1, 2, ...），用户可在设置中拖拽调整
...
- 同一供应商的不同统计维度可各自作为独立实例添加；同一供应商+同一维度可添加多次（如多个 DeepSeek 账号），配置不同的 API Key
- 同一供应商的多个实例共享一次 API 调用（相同 API Key 去重），各自提取对应字段
...
- **图标色彩模式**：单色 / 彩色
- **启动项**：是否开机自启
- **通知**：达到阈值时发送 macOS 系统通知，通知本体直接包含关键详情（如实例名称、当前用量/余额、阈值状态），点击通知打开独立的用量详情面板窗口（非依附式 Popover）
...
| `sort_order` | integer | 排序序号，升序排列，决定菜单栏槽位和 Popover 卡片的显示顺序。`sort_order` 相同时按创建时间排序 |
...
```
[菜单栏图标] ──左键单击──▶ [用量面板 Popover]
  │                                  ├─ 错误摘要栏（如有刷新失败）
  │  无配置: 显示 "?"                ├─ 各服务实例用量卡片
  │  1~2 实例: 显示对应槽位           │   ├─ 周期配额型：进度条 + 用量/上限
  │  ≥3 实例: 仅显示前 2 槽位         │   └─ 余额型：余额 + 当日用量(~)
  │  配额型槽位: 简称+进度条+数字     │       + 日均消耗（可选周期）
  │  余额型槽位: 简称+余额数字        ├─ [刷新按钮]
...
| 菜单栏空间有限，多槽位可能被系统截断 | 菜单栏最多同时显示 2 个槽位（固定 44pt 宽），超出实例需在 Popover 中查看；用户可手动禁用不关注的统计维度 |
