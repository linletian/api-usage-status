...
- **全部禁用状态**：所有已配置实例均被禁用时，菜单栏以灰色像素字模显示 `NO API`，左键点击仍可打开面板进入设置
...
- 点击菜单栏图标弹出浮动用量面板，按**服务实例**分组展示用量卡片。同一供应商的不同统计维度（如 MiniMax 文本 5h 与 MiniMax 非文本每日）各自独立一张卡片
- 每张卡片展示：
...
**首次使用提示**
- 应用首次启动且无配置时，菜单栏显示 `?` 符号；左键点击始终打开用量面板，面板内展示空状态（Empty State），包含「添加第一个服务」按钮，点击后进入设置窗口
