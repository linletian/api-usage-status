...
- **底行**：当前帧对应的循环文本（`%` / `%%` / `%%%`），使用 monospacedSystemFont 8pt，居中对齐。
- **颜色**：两行均使用 `dimColor`（#D6D0A0），即菜单栏在空状态下的统一淡化色。
- **槽位高度**：22pt，与正常状态槽位一致，确保状态切换时菜单栏整体高度不变。
- **宽度自适应**：分别计算首行和底行的文字像素宽度，取较大值作为最终宽度，保证两行居中后视觉对齐。
...
- 颜色使用 `dimColor`，与旧版 `?` 图标的颜色一致，确保空状态下的视觉连贯性。
